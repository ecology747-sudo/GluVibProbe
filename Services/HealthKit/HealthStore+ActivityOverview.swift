...
extension HealthStore {
    ...
    func exerciseMinutes(for date: Date) -> Int {                   // !!! NEW
        ...
        // Wir suchen im bestehenden Tages-Array nach dem Eintrag
        // für genau diesen Kalendertag.
    ...
    // ============================================================
    // MARK: - 7-Tage-Durchschnitt Exercise Minutes (appleExerciseTime)
    // ============================================================
    ...
    func fetchSevenDayAverageExerciseMinutes(endingOn date: Date) async -> Int {
        await withCheckedContinuation { continuation in
            ...
            // 7 Tage inkl. endDay → Start = endDay - 6 Tage
    ...
    // ============================================================
    // MARK: - 7-Tage-Durchschnitt Active Energy
    // ============================================================
    ...
    func fetchSevenDayAverageActiveEnergy(endingOn date: Date) async -> Int {
        await withCheckedContinuation { continuation in
            ...
            // Wir holen die letzten 30 Tage, filtern dann im Code auf 7 Tage,
            // damit die Helper auch für verschobene Tage (gestern/vorgestern)
    ...
    func fetchRecentWorkouts(limit: Int) async -> [HKWorkout] {
        if isPreview {
            // In der Preview lassen wir es leer → "No workout tracked yet"
            return []
        }
